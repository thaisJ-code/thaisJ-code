<div align="center">
  <h1 style="font-family: sans-serif; font-weight: bold; letter-spacing: 2px;">Hello,I'm Thaís! 👋🏻</h1>
</div>

<br>

<p align="center">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white">
  <img src="https://img.shields.io/badge/Audio_Processing-00599C?style=for-the-badge&logo=soundcharts&logoColor=white">
  <img src="https://img.shields.io/badge/Demucs-FF4B4B?style=for-the-badge&logo=audio&logoColor=white">
  <img src="https://img.shields.io/badge/Whisper_OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white">
</p>

Welcome to my audio and speech technology space. I specialize in processing audio signals, focusing on cleaning, source separation, and data formatting.

## 🛠️ Tech Stack & Tools
* **Language:** Python
* **Audio Separation:** Demucs (Noise removal and background separation)
* **Speech-to-Text:** Whisper (OpenAI)
* **Audio Analysis & Processing:** Librosa, Praat

## 🎯 Current Focus
* Developing clean, automated pipelines for speech and audio data analysis.
* Building professional freelancer workflows using Demucs, Whisper, and Librosa for clean audio and precise transcriptions.

---
*“Transforming audio into clear information.”*
